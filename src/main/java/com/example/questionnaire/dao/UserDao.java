...
public interface UserDao extends JpaRepository<User, Integer> {
    ...
    @Modifying
    @Transactional
    @Query(value = "update user set email = :#{#user.email}, phone_num = :#{#user.phoneNum}, username = :#{#user.username}, user.password = CASE WHEN :#{#user.password} IS NULL THEN user.password ELSE :#{#user.password} END  where user_id = :#{#user.userId}", nativeQuery = true)
