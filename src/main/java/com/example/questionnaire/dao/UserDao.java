...
public interface UserDao extends JpaRepository<User, Integer> {
    ...
    @Modifying
    @Transactional
    @Query(value = "update user set email = :#{#user.email}, phone_num = :#{#user.phoneNum}, username = :#{#user.username}, password = :#{#user.password} where user_id = :#{#user.userId}", nativeQuery = true)
